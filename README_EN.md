...
- 🎞️ **Video player frame + chapter theme layer (108 → 109)** — two pieces of user feedback: "four chapter cards on the same slab are monotonous" and "a shot holding a single video needs a player". `chapter-title-card` gains a **chapter theme layer**: per-chapter bg / ink / accent plus a line-art motif tied to the chapter's content (drawn on the L6 background at 0.6× parallax) and an accent rule under the name; the SHOTBOOK carries a chapter-theme line. New card **`video-player-frame`**: white-bordered evidence card + control bar (progress starts only once the play button is pressed and then runs on real playback time, with timecode and chapter marks) + title chip / source pill, plus a browser variant for web recordings; new SKILL.md §③ hard rule "single-video shots get a player" (no bare full-frame video, no bare white card; beds and videos embedded in compare cards excepted).

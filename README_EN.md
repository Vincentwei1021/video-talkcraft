...
**An agent skill for crafting high-quality voiceover-driven videos: word-level voiceover sync · 98 motion recipe cards · a 7-layer anti-slideshow shot system · triple-gate QA**
...
🖼️ [**Browse all 98 motion previews in the live Gallery »**](https://vincentwei1021.github.io/video-talkcraft/)
...
- 🔁 **9 cards ported from video-shotcraft (89 → 98)** — read all 157 product-video shot cards of the sister library [video-shotcraft](https://github.com/Vincentwei1021/video-shotcraft), filtered by "fills a voiceover gap / serves narration not product UI / passes motion subtraction / has a Remotion source", prototyped 18 in the lab and the user picked 9: word-slot cycle · lead-word zoom-assemble · title demote-to-label · scanline annotate · crash-zoom punch · B-roll freeze-frame annotate · unit-grid proportion · source converge · line-carry transition; each tsx is rewritten from its source into this library's self-contained contract and the reuse notes cite the source card (→ batch ◎ in `references/taxonomy.md`).
...
- 🎛️ **Motion workbench `workbench/`** — a CapCut-style post-production desk for finished videos: multi-track timeline,
  ...
  All 98 motion cards are parameterized (copy, colors, sizes, positions editable; timing vitals stay fixed).
...
- **Word-level voiceover sync** — `scripts/timestamps_cpu.py` aligns
  ...
  exact word.
- **98 motion recipe cards** — each with intent, parameters, known pitfalls,
...
| Content | Description |
| --- | --- |
| 98 motion recipe cards | Intent, energy, parameters, implementation notes, and known pitfalls — every card ships a self-contained Remotion tsx source (`template/cards/`, copy one file and go) plus a runnable HTML demo |
| Gallery | [Online](https://vincentwei1021.github.io/video-talkcraft/) or local (`open gallery/index.html`) — browse and autoplay all 98 previews, search by name/keyword |
...
├── demos/                      # 98 runnable HTML previews (+ shared lib with embedded SFX)

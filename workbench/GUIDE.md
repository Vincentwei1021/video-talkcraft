...
不用回到代码里改 tsx。109 张动效卡全部参数化，口播成片可一键拆成多轨工程逐项编辑。
...
不接任何外部工程也能跑：自带一个 14 秒的演示工程（上图），109 张动效卡全部可用。
...
109 张动效配方卡按[在线画廊](https://vincentwei1021.github.io/video-talkcraft/)的 7 个分类折叠：
强调标注 / 人物互动 / 数据信息图 / 素材呈现 / 运镜 / 转场结构 / 字幕花字（另有「工作台」自有卡）。
每张卡显示时长，「· 可调参」表示它有属性面板可调的参数——现在 109 张全部可调。

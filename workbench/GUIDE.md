...
不用回到代码里改 tsx。108 张动效卡全部参数化，口播成片可一键拆成多轨工程逐项编辑。
...
不接任何外部工程也能跑：自带一个 14 秒的演示工程（上图），108 张动效卡全部可用。
...
108 张动效配方卡按[在线画廊](https://vincentwei1021.github.io/video-talkcraft/)的 7 个分类折叠：
...
- 右侧显示合成尺寸与帧率（默认 960×540 · 30fps；拆解导入沿用原片参数；多轨拆解工程直接用接入工程的原尺寸与帧率，Player 自适应缩放）。
...
- **右键片段**：菜单里有分割 / 复制 / 删除，以及 **「导出透明通道 · MOV / WebM」**——只渲这一段（起点归零、时长精确、
  底透明），给剪映 / PR / AE 当叠加素材用；进度在右下角浮层，完成后「显示文件」。细则见 ⑦。
...
拆开之后工程文件一变（shots.json / 场景 / sfx.ts / 字幕）就自动同步，不用再点；你改过的文案 / 图层 / 删除保留。逐镜片段右侧带制作状态角标（占位 / 已实现 / 已渲 / 已过闸），占位镜画斜纹。

**skill 标准形态**（画布 = 工程原尺寸；时间轨上看到六类轨，幕级覆盖作为系统层隐去）：

| 轨道 | 内容 | 可以改什么 |
|---|---|---|
| 字幕 | 一段一个片段（工程 `phrases()` 给出的可见段：已扣掉画面大字同文的静音区、已做品牌名显示映射） | 文本、深浅底样式；样式用工程自己的 `SubtitleLine`，与成片同一份 |
| （幕级覆盖） | 工程 `Overlays`（黑震切帧 / 落幕压黑）——**系统层，不在时间轨显示**，导出照常带上，保证与成片一致 | 无（没有可编辑内容） |
...
时间轨上右键任意有画面的片段 → 「导出透明通道 · MOV」或「· WebM」：

| | MOV | WebM |
|---|---|---|
| 编码 | ProRes 4444 · `yuva444p10le` | VP9 · `yuva420p` |
| 适用 | 剪映专业版 / Premiere / Final Cut / AE 直接拖入当叠加层 | 网页 / Remotion `<OffthreadVideo>` / 体积小 |
| 体积 | 大（每秒几十 MB） | 小 |
...
边界：某张卡若把幕底画在**非根层**（嵌套的满幅 div），透明导出会保留那块底——先在属性面板把它的底色参数改成 `transparent`，
或告诉 agent 给该卡的幕底层加 `className="wb-shot-bg"`。音频片段没有画面，菜单项灰掉。同一时间只跑一个渲染（顶栏导出与右键导出互斥）。
...
- 阶段栏：①…⑧ 当前步高亮，hover 看该步的产物；右侧是镜头计数（占位 / 已实现 / 已渲 / 已过 / 过期 / P0-P1）和直播点（绿 = 实时连接中）。
  「⟳」按盘上产物重算一次；「⇣ 多轨（实时）」把本片拆成多轨并自动跟盘（或打开 `http://localhost:5199/?tracks` 直接进入）；
  不满足拆解契约的工程进不了多轨，右下角点名缺哪个契约模块（单轨「成片（实时）」2026-09-21 已下线）。

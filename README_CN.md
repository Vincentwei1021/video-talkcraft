...
的口播视频姊妹篇：一个把 Claude Code / Codex 变成口播视频动效工作室的 AI agent skill。
给它一份口播稿和一条成品配音，它在本机 CPU 上对齐字级时间戳、把每个语义拍写进 SHOTBOOK
分镜，然后用 [Remotion](https://www.remotion.dev/) 渲出电影感的解说成片——动态字卡、
证据截图、运镜、素排字幕、影视级音效，全部锁在人声上。

## ✨ 亮点

- **字级配音同步，纯 CPU**——`scripts/timestamps_cpu.py` 把口播稿对齐到音频
...
- **78 张动效配方卡**——每张有意图、参数、已知坑和可跑的 HTML 预览
  （`open gallery/index.html` 一页全览）。动态字卡、数据镜头、证据巡游、
...
| 你提供（输入） | skill 负责 |
| --- | --- |
| 口播稿（数字一律写汉字） | CPU 字级时间戳对齐，逐句质检标记 |
| 成品配音——任何 TTS 或真人录音（语音合成不在本 skill 范围内） | SHOTBOOK 分镜：语义拍、层矩阵、排版预算 |
| 可选的人物素材——绿幕或 alpha WebM（数字人生成不在范围内；CPU 抠像 + 人脸安全区工具已含） | Remotion 实现：四套全局系统（相机/视差/让位/环境）、转场、音效落位 |
...
**将本工具用于任何商业用途需事先获得授权**——X 私信
[@VincentWei93](https://x.com/VincentWei93) 或提 GitHub issue 联系。
...
- **[FireRedASR2](https://github.com/FireRedTeam/FireRedASR2S)**（经
  ...
  **[faster-whisper](https://github.com/SYSTRAN/faster-whisper)**——CPU 时间戳后端；

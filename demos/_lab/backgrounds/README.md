...
用户看过后反馈"运动稍微明显一点点"，页面默认改为 **1.6×**（页头可切 1× / 2.2× 对比）。落到 Remotion 时把基准值乘 1.6
（周期 ÷1.6 ≈ 12~25s、GLSL time 系数 ≈ 0.05~0.13）。
...
6. **速度**：网页背景默认速度普遍偏快（为了"活"），做幕底要慢下来。1× 基准：光球轨道 30~45s 一圈、光束 18~26s 一扫、GLSL time 系数 0.03~0.05、点阵亮波 14s 一过；
   用户实看后定为 1.6×（光球 19~28s、光束 11~16s、亮波 ≈9s）——比网页默认仍慢约一半，比 1× 基准"稍微明显一点点"。

...
2. **元素入场后就死了**：动画全是 entrance（opacity/translate 到位即冻结）——这正是 PPT"进入效果"的模型。动画感来自持续运动——**由场景相机的极缓推拉承担**，不靠元素自己抖（2026-09-04 用户定版）。
...
| 层 | 职责 | 典型动作 |
|---|---|---|
| L1 相机 | 带观众穿过空间 | **极缓推进 / 极缓拉出**（唯一要求的运动，2026-09-04 定版；横移/环绕/dutch 不再作为场景相机动作） |
...
| L6 环境 | 氛围 | 分幕色温（静态 token，可选）；呼吸 vignette / 扫光 / 曝光脉冲已删（2026-09-04） |
| L7 遮罩 | 控制可见性与取景 | 描边旅行、扫描线揭示、clip reveal |

层预算：**英雄时刻 4–6 层活跃；过渡镜头 2–3 层**；静息时刻允许只有相机一层在动（极缓推拉），
**不再要求主体 idle 与环境呼吸**（2026-09-04 用户定版：那套"处处在动"过了，不适用于口播——观众在听人说话、看字幕，画面的活只需一条几乎察觉不到的相机呼吸）。

## 2. 全局系统（先于任何场景实现，模板已提供代码）

代码在 `template/motion-systems/`，全部确定性（零 Math.random）、seek-safe。
**2026-09-04 用户定版（运动做减法）**：口播视频的运动系统只保留两件——**G1 相机极缓推进或拉出** + **G3 让位状态机**。
...
- **同起同收**（2026-09-05 用户定版）：相机曲线、实拍底床、素材缓推这类持续运动的 duration = 镜头时长，**写速率不写固定秒数**
...
- resolved 期**静置**（2026-09-04 用户定版）：`Live` 默认 `idle={false}`，元素落定后一个像素不动——画面的活由相机极缓推拉负责。
...
- 呼吸 vignette 与斜向扫光**已删**（2026-09-04，`breathe`/`sweep` 默认 false）；vignette 若要，是幕底配方里的**静态**一层（design-language §1.1），不是运动。
...
> 预算管"放多少"；**放哪、多大、怎么对齐**在 `references/layout.md`（2026-09-05 用户定版：栅格 / 间距令牌 / 对齐 / 居中 / 字阶 / 碰撞 / 底板配色 / 校验九项），两份一起核。
...
1. 英雄帧活跃层 4–6；任意静息帧**相机在动**（极缓推进或拉出；2026-09-04 起不再要求 ≥2 层）
...
8. **字与画同起同收**（2026-09-05 用户定版）：每镜最后一个退场结束 = 镜尾；镜尾之前不许出现"文字已退、底床 / 素材独自空转 >0.4s"；

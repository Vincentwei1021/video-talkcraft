...
| ④→⑤ 闸 | `preflight.py`「语义覆盖」段，四条：① **自我介绍 / 介绍他人 / 号召** 没有对应语义的卡 → FAIL（这三类有专设卡族、无替代物；**不分 main / sub**，标成 sub 也算）；② `数据` 主句且 `need` 含 `量化`，却没有能承载数字的卡（数据信息图类，或 `数据` / `强调` / `列举` / `对比` 语义——数字常以条目标签的形式落在并列卡上）→ FAIL；③ 其余 13 个语义（引用 / 对比 / 定义 / 步骤 / 列举 / 时间地点 / 机制 / 选择 / 过程演示 / 空间叙事 / 设问 / 金句 / 章节）→ WARN；④ 素材行声明了 V / 图 / 截图 却没有承接卡 → FAIL。承接卡 = 吃素材家族 + 类别不是转场结构 / 字幕花字 + 不是只承载人物的卡（`host-shrink-to-chip` 的角标窗里装的是讲者不是素材，算承接的话这条闸就只剩「连角标都没有」才触发）；该镜正文提到主题边框（ThemeFrame / 杂志框…）时降为 WARN。缺「蒙皮行」的镜不豁免（有素材或硬语义就 FAIL）。写「语义偏离」的放行 |
